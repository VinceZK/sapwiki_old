...
We already did PoC that technically proves BPC can directly consume data in ACDOCA through BW virtual info-provider. If we want to do futher testings, we need establish a complete testing environment. The enviroment should synchronize with BPC code pool in a high frequency and have a complete set of data for testing.  
...
If RTC and BPC own a dedicate testing environment(I give it a name TST), that is to buy and install a new S/4 enviroment. Then how code from both ER9 and KIW is synchronized to TST, and then merged into the main line still needs lots of configuration and operational effort. 

There is a solution that I think is most feasible. The solution is to request BPC developers to do double maintenance in KIW and ER9. Suppose the BPC main code is already sychornized to ER9, if bug or issues are tested out, then BPC developer do fix directly in ER9, and record the changes in a special change request, which will be excluded in the final release. Meanwhile, these fixes will be re-typed in KIW. Then next time, along with the new features, these fixes will be transported to ER9 and overwrite the existing ones. 
...
| Senarios/Num-of-Items  | 100,000 | 1,000,000 | 10,000,000 | 100,000,000 | 1,000,000,000 |
|------------------------+---------+-----------+------------+-------------+---------------|
...
The first column contains senarios we think is perfromance relevant during consolidation; The first row is the data volumn that represented by number of items of ACDOCA. Started from 100,000 lines, and ended with 1 billion lines(we can increase the end value if in real cases, there will be more line items in ACDOCA). The cells are filled with time spent in excuting a senario. Time should be differenciated by DB time, ABAP time, UI time, and the overall response time that end-user can feel. 

Besides, the corresponding memory and cpu consumption should be recorded in other tables. 

The performance result help us to understand the bottleneck, and know how to improve. Meanwhile, according to this discrete data, we could reduction a performance formular which can be used to estimate the reponse time and do hardware sizing in a given data volumn. 
