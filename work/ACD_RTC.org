...
Real-Time Consolidation provides consolidation foundations in the context of S4HANA. It is designed to move consolidation prepareation activities to the source(local finance), and eliminate data replications as much as possible. The efficiency and data quality are improved before data comes into the consolidation egine(BPC) for further processing. The consolidated results can also be stored back into S4HANA(ACDOCC).
...
5. Inter-Unit Reconcilation & Elimination
6. Posting
...
SAP delievers 2 examples of foundation views. With one on ACDOCA for actual data, and the other on ACDOCP for planning data. They project fields that are usually realted to financial consolidation, and do filtering on ledgers.  

**** Modeling
The modeling includes all master data preperation, customization data definition, and consolidation model definition. There is a specific modeling tool which collects the foundation view, group chart-of-account, group fiscal year variant, and consolidation dimension to gerenate consolidation views.
...
**** Posting
Posting is used to save data into ACDOCC. It includes posting APIs and related manual posting UIs. To achieve better performance, the posting APIs are utlized using AMDP. So that the consolidation result is not necessary to send to ABAP and return back to the HANA. 

The posting may also need to call document validation to control the quality when data is entring into ACDOCC. Doucment type is supported which allow to do some customizations to control how posting is done for different purpose. 

For details, see [[https://wiki.wdf.sap.corp/wiki/display/ERPFINDEV/SDD-Postings][SDD Posting]].

**** Inter-Unit Reconcilation & Elimination
We support reconciliation and elimination so that group reports can be made as easily as possible. The reconciliation can be as simple as only a reconciliation report without any assignments. It is also possible to leverage existing SAP Inter-Company Reconcilation solution for a full feature set. 

Elimination shares the similar pairing rules with reconciliation, but it also contains how to do automatical postings and record differences. 

Both reconciliation and elimination can be run on-the-fly. 
**** Reporting
There are some genaric reports developed using Fiori so that on-the-fly run can be supported. User can aslo define CDS views or BW Queries on consolidation views. Those queries can be then either consumed by AO, or any other reporting tools.
...
1. Consolidation Foundation, which includes modeling, currency transaltion, consolidation rule definition, and posting.
2. Data Collection, which includes data release and flexible upload.
3. Validation Engine, which can be decoupled from RTC to be a more genaric component. 
4. BPC Integeration, which includes all BW and HANA artifacts for the integeration with BPC.

The consolidation foundation optionally depends on BPC which belongs to the SAP_BW component if the model is created with BPC integeration option. Validation Engine depends on SAP BRF+/HRF which belongs to the SAP_BASIS component. And they are all in NW 752 version. 

There are also dependencies on HANA level. For the BPC integeration part, it depends on the HANA calculation views in the delivery unit "HCO_ERP_SFIN_RTC". And SAP BRF+/HRF also depends on the delivery unit "HCO_RULE_FW".
