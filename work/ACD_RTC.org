...
** Abstract
Real-time Consolidation(RTC in short) provides the foundation for the financial consolidation on S4HANA(OP&Cloud) platform. It provides the high quality data for consolidation in a real-time fashion. RTC is designed to be easily integrated with BusinessObject Planning and Consolidation(BPC in short) and all other possible consolidation solutions. Existing consolidation solutions can leverage RTC to consume S4HANA data in a much easier way.     
...
**** Data Validation
Data validation is to control the data quality for consolidation. It can be happen during data release as well as consolidation process. 

**** Currency Translation
Currency translation is used to convert the local currency amount to the consolidation currency amount. As different types of accounts use different translation methods and rates, translation differences thus could be happen can will recorded according to the translation rules predefined. 
...
| *How does the interaction between initiator and executor take place?* | At period-end, the group accountant opens current period for consolidation data collection. She open the Fiori App called Data Mass Approval, and choose current period, and then click the open button. |
| *Under which conditions / environment does the interaction take place?* | At period-end, the group accountant initiates and 10,000 Data Release Requests will be generated for 1000 sites. |
| *Result of activity*           | Sites are notified, and data release requests are generated. |
| *KPI*                          | Data Release Requests are correctly generated.     |
...
| *Under which conditions / environment does the interaction take place?* | At period-end, 1000 sites will release their data. 1000 Validation rules for each would be run against the ACDOCA table. |
...
| *Result of activity*           | Currency amounts are correctly converted and saved. |
| *KPI*                          | Currency Translation can be run on-the-fly, which means at maximum time of 10 seconds. |

*** TODO Product Standards
...
| Business Logic Layer                   | ABAP, AMDP, CDS, HANA Calculation View               |
...
| Attachment Service | PLM DMS team    | Yes   | must   | Fiori Attachment Service is a reusable component for Firoi apps that need to upload and store documents into S4HANA system. |
| Design Studio   | EPM team        | Yes   | should | Design Studio is used to create queries. The query can be opened via various analysis tools, like: AO, Fiori Apps, and so on. It is appointed by S4HANA guideline for the only query builder, and will replace BEx in future. |
...
*** TODO Cross-Release Compatibility
...
1. BPC is the only legal consolidation and planning product in SAP. RTC may be bundled with BPC for sales and marketing.
2. For the future Cloud usage, RTC would support distribution system architecture that allows applications on HCP can be easily integrated.
3. RTC should also provide consolidation foundations for other consolidation solutions that would consume data in S4HANA. 
...
Real-Time Consolidation provides consolidation foundations in the context of S4HANA. It is designed to move consolidation preparation activities to the source(local finance), and eliminate data replications as much as possible. The efficiency and data quality are improved before data comes into the consolidation engine(BPC) for further processing. The consolidated results can also be stored back into S4HANA(ACDOCC).

The main components in RTC are:
1. Consolidation Modeling
2. Data Collection
3. Data Validation
4. Currency Translation
5. Inter-Unit Reconciliation & Elimination
6. Posting
Which are also described in the following diagram:

#+CAPTION: Overall Architecture Diagram
[[../image/OverallArchitectureDiagram.png]]

**** Foundation View
Foundation view acts as the consolidation data foundation, on which all the consolidation views are generated. It is created manually by user, either CDS view or HANA Calculation view. It should only contain the essential logic like project fields and data filtering. In other words, it boundary the data for consolidation. 

SAP delivers 2 examples of foundation views. With one on ACDOCA for actual data, and the other on ACDOCP for planning data. They project fields that are usually related to financial consolidation, and do filtering on ledgers.  

**** Modeling
The modeling includes all master data preparation, customization data definition, and consolidation model definition. There is a specific modeling tool which collects the foundation view, group chart-of-account, group fiscal year variant, and consolidation dimension to generate consolidation views.

**** Consolidation View
Consolidation views are CDS views and HANA calculation views generated by the modeling tool based on the foundation views and other meta definitions, like, Chart of Account, Fiscal Year Variant, consolidation dimension, and so on.

**** ACDOCC
ACDOCC is the consolidation journal table which are mainly used to store consolidation journals and data provisioned by flexible upload.

The ACDOCA and ACDOCP provide source local financial data for consolidation. Foundation views can be created on these tables. 

**** BW Integration
It is necessary to provide interfaces for BW(BPC) consumption, as we can leverage a lot of existing analytic and consolidation features which BW already has. HANA calculation views are generated based on the consolidation view, which are then consumed by BW virtual provider or composite provider. For the data write-back, an ABAP class is provided to allow BPC write-back the consolidation result to ACDOCC. 

**** Validation
Validation is used to control data quality during the consolidation process. The validation engine is designed as a standalone component, which can be used not only in financial consolidation, but also in other FI applications. Validation rules can be defined, stored, and run in validation engine. 

Total validation is used to validate data on aggregated level, which may already be persisted in DB tables. The rules used for total validation are called analytic rules. You group rules in validation engine and assign them to a call-point. 

Document validation is used to validation line item data, which is transient. For example, I want to validate a document before posting based on rules defined by myself. The rules used for document validation are called transactional rules.  

**** Data Collection
Data is collected from all the subsidiaries through various ways. In ideal cases, all the companies run their financial transactions in one S4HANA instance, or the transactions are synchronized into a Central Finance System. Then we only need to control how data is released for consolidation by adding timestamp filtering and do data validations by request total validation component. 

But in real cases, data is not that integrated. Some subsidiaries have to upload aggregated data through workbooks. The collection process won't be in such an easy way. So flexible upload is provided so that external entities can provide their data to the consolidation system. Technically, it supports spreadsheets upload, manually entering, and web services APIs. The data is stored in ACDOCC by calling the posting APIs. Validation and data release control should also work on these kinds of data provision. 

**** Currency Translation
Currency translation is needed when local currency is different with the consolidation currency. The amounts have to be translated before further consolidation activities. It reads the data through consolidation views, translate, and save the result into ACDOCC through posting APIs. 

Currency translation can be invoked in multiple entry points. If local accountants are required to submit the translated amounts, then they run translation during data release; Group accountants can run translation either on-the-fly in reporting, or in consolidation processes before the elimination step. 

**** Posting
Posting is used to save data into ACDOCC. It includes posting APIs and related manual posting UIs. To achieve better performance, the posting APIs are utilized using AMDP. So that the consolidation result is not necessary to send to ABAP and return back to the HANA. 

The posting may also need to call document validation to control the quality when data is entering into ACDOCC. Document type is supported which allow to do some customizations to control how posting is done for different purpose. 

For details, see [[https://wiki.wdf.sap.corp/wiki/display/ERPFINDEV/SDD-Postings][SDD Posting]].

**** Inter-Unit Reconciliation & Elimination
We support reconciliation and elimination so that group reports can be made as easily as possible. The reconciliation can be as simple as only a reconciliation report without any assignments. It is also possible to leverage existing SAP Inter-Company Reconciliation solution for a full feature set. 

Elimination shares the similar pairing rules with reconciliation, but it also contains how to do automatic postings and record differences. 

Both reconciliation and elimination can be run on-the-fly. 
**** Reporting
There are some generic reports developed using Fiori so that on-the-fly run can be supported. User can also define CDS views or BW Queries on consolidation views. Those queries can be then either consumed by AO, or any other reporting tools.

**** Consolidation Dashboard
Consolidation Dashboard provides a central place to view all statuses of consolidation processes, and also an entry point to access all modeling, customization, preparation, and running applications.

**** BPC
BPC consumes data through a virtual provider and the HANA views generated on consolidation views. From BPC's perspective, the data is already for consolidation. The consolidation engine of BPC access the real-time data, calculate and write-back to ACDOCC by calling the write-back class.
...
**** Rule framework
Consolidation rule is an important part of financial consolidation. In S/4 guideline, BRF+ is the only permitted rule framework. But BRF+ is an ABAP rule framework, if you want to leverage HANA, you can only use HANA Rule Framework(HRF) through BRF+ interfaces. BRF+ and HRF already planned a road-map to make the 2 frameworks merged into one. 
...
| <15>            | <50>                                               |
| *Decision*      | Develop Validation on BRF+/HRF                     |
| *By*            |                                                    |
| *Date*          | <2016-12-02 Fri>                                   |
| *Description*   |                                                    |
...
1. *Consolidation Dashboard*: for data collection, currency translation and Inter-company reconciliation.
...
Fiori should be the first option according to the S/4 HANA guidelines. But considering the team capability, we are lacking of Fiori development resources. If we choose ABAP dynpro, although we can gain some efficiency, we will lose the cloud target. Besides, the ABAP dynpro is less attractive.   
...
|----------------------+----------------------------------------------------+----------------------------------------------------|
| Cons. Hierarchy      | Support both version and time dependent.           | Create from scratch for both Backend & UI           |
...
#+CAPTION: Software Component Structure
[[../image/SoftwareComponentStructure.png]]

There are 4 components in RTC:
1. Consolidation Foundation, which includes modeling, currency translation, consolidation rule definition, and posting.
2. Data Collection, which includes data release and flexible upload.
3. Validation Engine, which can be decoupled from RTC to be a more generic component. 
4. BPC Integration, which includes all BW and HANA artifacts for the integration with BPC.

The consolidation foundation optionally depends on BPC which belongs to the SAP_BW component if the model is created with BPC integration option. Validation Engine depends on SAP BRF+/HRF which belongs to the SAP_BASIS component. And they are all in NW 752 version. 

There are also dependencies on HANA level. For the BPC integration part, it depends on the HANA calculation views in the delivery unit "HCO_ERP_SFIN_RTC". And SAP BRF+/HRF also depends on the delivery unit "HCO_RULE_FW".
...
**** Validation Engine is built upon HRF
| <20>                 | <50>                                               |
| Description          | Consolidation validation is built upon HRF which is in parallel development. The development plans of the 2 teams must be highly synchronized, or it will impact delivery of Real-time Consolidation. However, the 2 teams are in 2 different locations(one in Shanghai China, the other is in Israel), which introduces the inconvenient in communication. |
| Impact(for customer) | Some functions can not be released in 1709 release. The worst case is the whole validation component cannot be released. |
| Impact Rating        | High                                               |
| Risk Probability     | High                                               |
| Mitigation Activity  | Improve communication efficiency by on site visiting, weekly sync meeting, share customer's requirements. |
| Responsible Person   | PO, Arch, and Program level management team        |
| Due Date             | null                                               |
...
7. [[https://wiki.wdf.sap.corp/wiki/display/ERPFINDEV/SDD-Elimination][Reconciliation & Elimination]]
