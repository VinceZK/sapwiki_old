...
In this tab screen, you can create a consolidation version, which you just assign a unique value combination of =Data Category=, =Account Principle=, and =Variant=. In the sceario with BPC, you must also map a BPC Category to one of your version combinations. 
...
When you click the "Add", a popup dialog will allow you to add a new version. You give an unqiue BPC category. If it doesn't exist, it will create one.

~The master head table for BPC category is FCOMC_CATEGORY which belongs to the IBPF team. We share the categroy master data for future planning and consolidation integration. If you add a new BPC category, it will insert a new entry into table FCOMC_CATEGORY.~
