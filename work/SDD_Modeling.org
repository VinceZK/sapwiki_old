...
The first step is to create a foundation CDS view on ACDOCA(and other tables if necessary) in HANA stuido. The foundation view projects fields from ACDOCA (and join or union other data sources), apply filters(where conditions), do field mappings and alignments. We use CDS modeling tool as the foundation view creation is exactly DB modeling activities. It is not necessary to build a dedicate DB modeling tool for consolidation.

The performance of the foundation view should be guarantee, as most of the consolidation tasks afterwords are pumping data from it. 
...
Version combinations can be defined to allow parallel consolidations based on the same model. For example, actual and plan, US GAAP and IFRS. You can use 2 fields: =Data Category= and =Version= to define various combinations. Each combination can have its specific customizations, like consolidation frequency, different validation method, and so on.
...
After successful activation, the consolidation model can run consolidation tasks. CDS views could be generated for each version combination if "Integration with BPC" is checked. These CDS views are called =Consolidation Area Views= which are created on foundation view by adding additional fixed value columns. If "Integration with BPC" is not checked, then the foundation view will be regarded as the area views.

~If possible, there will also a HANA calculation view be generated on consolidation area CDS view for the consumption from BPC.~

The =Consolidation Area View= will be the basis for all the consolidation tasks. HRF Vocabulary can also be formed based on it so that rules can be defined. Consolidation journals generated from tasks are then posted to ACDOCC,  which can then be consumed directly by S/4 applications or be accessed by BPC for further consolidation tasks. 

Above steps are involved in =Branch1= which is data modeling for RTC in S/4. The other branch for BPC is off-the-peg. User can just use HANA Studio to create calculation views and use composite info-providers to let BPC directly consume data in ACDOCA. The consolidated results can then be write-back to S/4 through an ABAP write-back class. 
...
Details are described in following building blocks:
...
1. *Data Stream* defines the consolidation foundation views, and journal tables to persist consolidation result.
2. *Integration* defines how BPC is integrated to access data in S/4.
3. *Version* defines the version combinations, and version specific settings.
4. *Assignment* assigns FI objects like consolidation ledger, CoA, and Fiscal Year Variant.
5. *Admin* read-only administrative information. 

#+Caption: Consolidation Model UI Tab 1
[[../image/ConsEnvDefinationUI_1.png]]

User can define multiple consolidation models, with each have 4 different status: =Active=, =Inactive=, =Saved=, and =Error=. Only active consolidation models can be run. 

In the detail screen of a consolidation model, you can state its dimension. Currently, only 1 dimension is supported. User can select among =Company=, =Profit Center=, and =Cost Center= dimensions. Each dimension has a combined partner unit assigned. If using ACDOCA&ACDOCC, partner dimension is selected by default.

In the =Data Stream= tab, a CDS view on ACDOCA is chosen as the consolidation foundation view. This is the mandatory view that contains all the actual financial journals. Other views can be added for different consolidation tasks. As in the initial release, only consolidation data view is involved, so the UI is designed as a form just for the simplification. But under the hood, you should be awared a consolidation model could have multiple data foundation views. 

The consolidation journal table currently is fixed as ACDOCC. It is supposed in future use can assign its own journal tables to persist consolidation results. Consolidation area view can be generated automatically based on the mapping information between consolidation data view and consolidation journal table. There will be a data mapping screen in future releases, but currently, the foundation view and ACDOCC are supposed already mapped through the same field names. 

#+Caption: Consolidation Model UI Tab 2
[[../image/ConsEnvDefinationUI_2.png]]

=Integration= tab contains settings on how BPC is integrated with S/4. Currently there are 2 check boxes. If "Integrate with BPC" is checked, then new area views are generated on the foundation view. These area views contains fixed value columns, like "model", "category", "version", and so on. 
