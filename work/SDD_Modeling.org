...
The BPC consolidation views will be union-ed under a composite provider, together with a dummy virtual provider only for write-back. User could still need to prepare info-objects(virtual or physical) for the dimensions in the composite provider. 
...
3. 
4. 
5. If all involved company codes are mapped to companies? (For other dimension,like profit center, cost-center, there would be a mapping table planed).
6. If all fields defined in foundation view have counterparts in ACDOCC?
...
Not all the operational accounts need to be involved in a consolidation. There should be a place to allow user to define which account numbers should be involved for certain company codes. 

This can be achieved using the general object: =Selection=. A special composite selection is reserved with the name "SELECTION001". Under the composite selection, user can add multiple selections to compile his accounts selection.
...
You must assign a consolidation Chart of Account, a Group Fiscal Year Variant, a Consolidation Entity dimension and its partner unit. Besides 2 BPC categories must be defined, one is for the final consolidation, and the other is for the preliminary consolidation run. If the entered category name doesn't exist, it will create the category with the given name. 
...
In this tab screen, you can add a new version by first assigning a unique =BPC Category= value, and then assign a value combination of =Consolidation Ledger=, =RTC Category=, and =Version=. The above three version fields are default assigned. In future release, you can define your own version fields combination as you like. Also, by default, the first 2 lines with =BPC Category= value equals "Actual" and "Prelim" are mandatory. You can not delete them, and you must assign a valid value combination for it.  
