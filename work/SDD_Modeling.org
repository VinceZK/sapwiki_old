...
6. Define Data Foundation Views and Consolidation Views;
...
1. CT in Consolidation: Currency is translated in consolidation systems(like RTC or BPC);
2. CT in G/L Accounting: Currency is translated in G/L Accounting system, and conventially, happens during document posting.
...
~You must first enter the HANA package where the foundation view resides. Bear in mind, the HANA DB user "SAP<SID>" must have the read/write access to the HANA package of FI Journal. As later on, it will generate HANA Calculation views into the same package. And the HANA package length should not exceed 40 characters.~ 
...
4. Default Flow Type exists(in case BPC integeration);
...
12. A consolidation version must at least has a data category, a ledger, and a variant, each combination must be unique;
13. If the model is BPC integrated, a unique BPC category must be mapped to each consolidation version;

Following criterias must be additionally fulfilled before activating the model:
1. In case BPC Integration, a BPC environment and model must be given;
2. The assigned BPC environment and model must not be assigned to other RTC model;
3. The settings between the BPC model and RTC model must be consistent;

**** Model Activation
When activating a model, model validation should be run before following stuff could be generated:
1. Consolidation views;
2. Default document types and number range interval 00;
3. BRF+ Application(/RTCART/<model name>) and add validation views into it. 
 
Once model is activated, following fields cannot be changed again:
1. Currency translation mode;
2. Dimension;
3. Group Chart of Accounts;
4. Group Fiscal Year Variant;
5. Default Flow Type;

Following changes need view regeneration when activating:
1. Add a new consolidation version;
2. Change foundation views;

Data Category cannot be deleted if it is already used in a consolidation version. 
