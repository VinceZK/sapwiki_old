...
4. If all involved company codes are mapped to companies? (For other dimension,like profit center, costcenter, there would be a mapping table planed).
5. If all fields defined in foundation view have countparts in ACDOCC?

There are quite a lot of consolidation views generated automatically. These views are for different consolidation and validation tasks. In general, these views are categorized into 2 sets: HANA calculation views for BPC's comsumption, and CDS views for RTC's own comsumption. 
The BPC consolidation views will be unioned under a composite provider, together with a dummy virtual provider only for write-back. User could still need to prepare info-objects(virtual or physical) for the dimensions in the composite provider. 
...
1. If entity defination is consistent between RTC and BPC?
2. If CoA and Fiscal Variant defination are consistent between RTC and BPC?
...
The RTC Modeling Tool is used to define a Real-time Consolidation model object, and generate necessary consolidation views. It also permits the consistency of a consolidation context that run accross S/4 and BPC. The RTC Modeling tool contains following parts:
...
**** Modeling Validation Method                                        :Lynn:
As decribed above, modeling validation happens before consolidation view generation and model activation. There wold be no less that than following rules should be checked:
...
For generating CDS view, there are existing APIs to allow ABAP generates CDS views. Transportation is also utilized in the set of APIs which allow the generated CDS views can be transported from DEV to QA, and to Product system.
...
For generating HANA Calculation Views.... Ask Marvin and Steve.
...
You must assign a consolidation Chart of Account, a Group Fiscal Year Variant, a Consolidation Entity dimenstion and its partner unit. Besides 2 BPC categories must be defined, one is for the legal consolidation, and the other is for the prelimnary consolidation run. If the entered category name doesn't exist, it will create the category with the given name. 
...
By default, Real-time Consolidation needs BPC for the complete consolidation functionalities. So the checkbox of "Integrate with BPC" is always checked. It is possible for those customers who doesn't have BPC, can still run some simple consolidations directly in S/4. Then they should uncheck the option. And this model will not generate any BPC consolidation views. 
...
Click the "OK" button will do the consolidation views generation work. It will run validations first. The validation result will be shown in the next screen(300) as a popup dialog(screen 900). If everything is ok, the dialog won't be shown. 
