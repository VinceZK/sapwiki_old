...
Unlike Data Category, Accounting Principle is assigned with consolidation rules rather than data. For example, you assign different currency translation methods to different accounting principles. It acts a media for sharing  consolidation rules among different consolidation versions. 

**** Variant
Variant is the third version field. It acts as the additional dimension used to differentiate consolidation versions which have the same Data Category and Accounting Principle. 

You can define customizations like exchange rate indicators on variant. Becides, there are also attributes alone with variants like data release mode, whether adjustment release request is needed.  
...
*** Flexible Upload
When the local accountant submits his local data with Flexible Upload, he only chooses a target Model and a Data Category. When the data is saved in ACDOCC, only Data Category field is filled with value "Actual", Leger and Variant fields are kept as empty. The following persistence layout applies:
...
Exchange Rate Indicator is maintained under Variant. You can achieve different Exchange Rate Types in different Data Categories. For example:
| Model  | Variant | ERI | Type |
|--------+---------+-----+------|
| RTCCTS |     100 |   1 | 1001 |
| RTCCTS |     100 |   2 | 1002 |
| RTCCTS |     100 |   3 | 1003 |
...
| Variant | Description | Data Release Mode              | PP Depands |
|---------+-------------+--------------------------------+------------|
|     100 | Final       | 0:Data Release is Required     |            |
|     200 | Preliminary | 9:Data Release is not Required |        100 |

To keep the data consistency, there must have following assignment logic when Data Category and Account Principle are the same. 
1. Variant 200 can only be assigned if its PP depands variant exist under the same Data Category and Account Principle;
2. Variant 200 *cannot* be assigned if there exist more than one variants with mode "0"
3. There could be multiple mode "9" variants with only one mode "0" variant under one Data Category and Account Principle combination. 
...
| FINAL2   | Actual2   | USGAAP     |     110 | Final cons for US GAAP          |
...
**** Run CT for C100
I first run RTCCT under version *Actual1/LGAAP/200* for company *C100*, it will call CT method *CTM01* and generate following results in ACDOCC:
| Model  | D.Cate  | Ledger | Variant |  Period | Entity | LC CNY | GC JPY |
|--------+---------+--------+---------+---------+--------+--------+--------|
| RTCCTS | Actual1 | C1     |     200 | 2016008 | C100   | 8,000  | 48,000 |

~Notice: for preliminary categories, each time you re-run CT, it will delete the former CT documents.~

Then next time, I run RTCCT under version *Actual1/LGAAP/100*, it will generate following results in ACDOCC:
| Model  | D.Cate  | Ledger | Variant |  Period | Entity | LC CNY | GC JPY |
|--------+---------+--------+---------+---------+--------+--------+--------|
| RTCCTS | Actual1 | C1     |     200 | 2016008 | C100   | 8,000  | 48,000 |
| RTCCTS | Actual1 | C1     |     100 | 2016008 | C100   | 8,010  | 48,060 |

The BPC Category "FINAL1" and "PRELIM1" will read seperate data set of current period. But they share the same data set of prior periods. Then when open the next period (2016009), it should delete the lines under variant "200", and clear the value of variant "100". The data looks like bellow:
| Model  | D.Cate  | Ledger | Variant |  Period | Entity | LC CNY | GC JPY | DEL |
|--------+---------+--------+---------+---------+--------+--------+--------+-----|
| RTCCTS | Actual1 | C1     |     200 | 2016008 | C100   | 8,000  | 48,000 | X   |
| RTCCTS | Actual1 | C1     |         | 2016008 | C100   | 8,010  | 48,060 |     |
Now, "PRELIM1" and "FINAL1" share the same data of period 2016008.
