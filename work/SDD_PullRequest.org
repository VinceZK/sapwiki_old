...
11. =DIMEN=, =RVERS=, and =BUNIT= are only used for UC-CS, which are corresponding to the Model, consolidatin version, and consolidation entity.
...
The Data Submit Cockpit is divided into 2 sections: The permanant parameters in the upper area, and the package contents in the lower area. The permanant parameters are fixed to the following 5:
1. Consolidation Model(Dimension if it is UC-CS);
2. Consolidation Version(BPC Category if it is for BPC);
3. Consolidation Enity(Consolidation Unit if it is UC-CS);
4. Group Fiscal Year;
5. Group Fiscal Period.

A package is defined as data of one entity for one period. For example, the data of entity 300000 for period 2016008. A package can be further divided into sub-packages, usually based on different types of accounts. For example, All account payables & account receivables are defined as a sub-package. All sub-packages together should cover the complete set of the main package, however, intersections among sub-packages are allowed.

You click the name of a sub-package can navigate you to the sub-package detail page. However, you can also directly operate on the list for these important operations listed bellow:
1. Locked/Unlocked: Whether this sub-package is locked or not by the group accountant, click to switch between "Lock" and "Unlock".
2. Control: Control runs the data validation before sub-package is submitted. If it doesn't pass, submission is not allowed. You click to run the control directly or open the detail page of the su-package's control.
3. User Status: Status changing triggered by users, like: Initial, Submitted, Approved, Rejected, Deferred; Click to pop-up a dropdown list to swtich the status. 
...
5. Variation: Each time a user logons Data Submit Cockpit, it detects whether there is variation till last consolidation run in the same period. Variation means data changes after consolidation is finished for the period. Click the status will navigate you to the detail of the changed data in the "Submit History" tab.

The main package itself also has user status, system status, and control status. Once all its sub-packages are approved, can the package itself get approved. The control of the main package is not just sumarization of all sub-packages' controls, it is a seperate rule set.

The buttons on the toolbar area operates on package level(not sub-package level). The blue buttons are only shown when group accountants access the application, and the orange ones are only shown for local accountants.

** Data Submit Cockpit Controls
...
Each rule result has a trrific light to state it is passed or not. Red light means failed, green light means pass. Yellow means failed but comments are provided which is then exampted. You can run a rule seperated by click the icon _Run This Rule_; you can hover over the link _Detail_ to view more detail of the rule result; you can supply or read comments and attachments; and you can dropdown the drill through reports to get further investigations. 

** Data Submit Cockpit Comments
...
If you are in the comment tab of the main package, then you can view all the comments which belong to its sub-packages, its submit requests, and its validation logs. User can fitler to display. If you are in the comment tab of the sub-package, you can only view comments that belong to its submit requests and validation logs. 
...
In the left panel, you can view all the data submit requests related to the (sub-)package for a cerntain period. A data submit request is timestamp annoted. You can compare the difference between either 2 requests. In the right panel, from the dropdown box, you select the beginning. For example, you select "Opening", which is the startpoint of the period, usally the first second of the period. Then you click any one of the item in the left list, it will show you the difference between the 2 timestamp. 
** Data Submit Cockpit Data Entry 
#+CAPTION: Data Submit Cockpit Data Entry Screen
[[../image/DataSubmitCockpit20.png]]  
Click the link of each sub-package navigates you to this data entry page, which contains the actual data in B/S format. The report contains amounts of prior periods in local currency, you can also run on-the-fly currency translation to get the group currency amount. You can switch the group currency in the right-up corner. 

"Submitted Amount" means data already submitted in the previous submit requests. "To be Submitted" means the delta amount assigned to current tobe submitted request. The amount is calculated by the total amount minus the "Submitted Amount". 
