...
The UI divides into the left panel and the right panel. The left panel contains the list of all rules defined, user can type words to do fuzzy search. Click a rule in the list, details of the rule will be shown in the right panel. You can also create a new rule by click the "New" button in the bottom bar of the left panel, which then will display an empty rule in the right panel.
