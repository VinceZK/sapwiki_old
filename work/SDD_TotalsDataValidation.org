...
IF Account(RACCT) is between "5000" and "6000" THEN PENTITY is not equal to ''

=Descision Table=
| BALANCE_CHECK | MSGTY | MSGID         | MSGNO | MSGV1 | MSGV2 | MSGV3 | MSGV4 |
|---------------+-------+---------------+-------+-------+-------+-------+-------|
| true          | 'S'   | MSG_CLASS_XXX |   003 | XXX   | XXX   |       |       |
| false         | 'E'   | MSG_CLASS_XXX |   004 | XXXX  | X     |       |       |


**** If the deviation of account 4101001 for fully consolidated partner units as a total is bigger than $2 million and bigger than 25% of the prior period, then error. 
=Prerequisite=

If Account is '4101001'

=Check=
1. Sum of the amount for all fully consolidated partner units as =ThisConsAmount=: Sum of amount where account = '4101001' and period = <current period> and consolidated = true.
2. Sum of the amount for all fully consolidated partner units as =PriorConsAmount=: Sum of amount where account = '4101001' and period = <current period -1> and consolidated = true.
3. Create an alias =DEVIATION_CHECK= as =ThisConsAmount= < 2,000,000 and =ThisConsAmount= > =PriorConsAmount= * (1 + 25%)

=Descision Table=
| DEVIATION_CHECK | MSGTY | MSGID         | MSGNO | MSGV1 | MSGV2 | MSGV3 | MSGV4 |
|-----------------+-------+---------------+-------+-------+-------+-------+-------|
| true            | 'S'   | MSG_CLASS_XXX |   005 | XXX   | XXX   |       |       |
| false           | 'E'   | MSG_CLASS_XXX |   006 | XXXX  | X     |       |       |
