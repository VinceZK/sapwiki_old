...
As HANA SQLscript supports little on dynamic programming. The only way to realize currency translation customization to runtime is through automatically generating CDS views and ADMP class. The detail field mapping logic and data flow can be find in following diagram. 
...
**** Selection CDS View      :Chang:Marvin:
